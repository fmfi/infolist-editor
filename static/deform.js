...
var deform  = {
    ...
    processSequenceButtons: function(oid_node, min_len, max_len, now_len,
        ...
        var $ul = oid_node.find('.deformSeqContainer').not(oid_node.find('.deformSeqContainer .deformSeqContainer'));
        var $lis = $ul.find('.deformSeqItem').not($ul.find('.deformSeqContainer .deformSeqItem'));
        var show_closebutton = now_len > min_len;
        var show_addbutton = now_len < max_len;
        $lis.find('.deformClosebutton').not($lis.find('.deformSeqContainer .deformClosebutton')).toggle(show_closebutton);
        oid_node.find('.deformSeqAdd').not(oid_node.find('.deformSeqContainer .deformSeqAdd')).toggle(show_addbutton);
        $lis.find('.deformOrderbutton').not($lis.find('.deformSeqContainer .deformOrderbutton')).toggle(orderable && has_multiple);
     },
