...
class PrilohaVPCharakteristiky(Priloha):
  ...
  def render(self, to_file):
    rtf_skin = resource_string('ilsp.osoba', 'templates/charakteristika-skin.rtf').split('CHARAKTERISTIKA_CORE')
    to_file.write(rtf_skin[0])
  ...
  def get_core(priloha):
    ...
    rtf_skin = normalize_crlf(resource_string('ilsp.osoba', 'templates/charakteristika-skin.rtf')).split('CHARAKTERISTIKA_CORE')
    with closing(StringIO()) as f:
        priloha.render(f)
        s = f.getvalue()
    s = normalize_crlf(s)
